...
const applyDiscount = (subtotal, type, value) => {
  const v = Number(value) || 0;
  if (!type || v <= 0) return subtotal;
  if (type === "pct") return Math.max(0, subtotal * (1 - v/100));
  if (type === "amount") return Math.max(0, subtotal - v);
  return subtotal;
};
...
const T = {
  ...
  quizasReponer:{fr:"Peut-être à réapprovisionner",es:"Quizás toca reponer",en:"Maybe time to restock",it:"Forse da riordinare"},
  quizasReponerSub:{fr:"Vos modèles habituels que vous n'avez pas recommandés depuis un moment — disponibles maintenant.",es:"Tus modelos habituales que hace tiempo que no pides — disponibles ahora.",en:"Your usual models you haven't reordered in a while — available now.",it:"I tuoi modelli abituali che non riordini da un po' — disponibili ora."},
  haceTiempo:{fr:"Pas commandé depuis ~%d mois",es:"Sin pedir hace ~%d meses",en:"Not ordered in ~%d months",it:"Non ordinato da ~%d mesi"},
  descubreModelos:{fr:"À découvrir",es:"Descubre",en:"Discover",it:"Da scoprire"},
  descubreModelosSub:{fr:"Des modèles que vous n'avez pas encore essayés, disponibles à l'envoi.",es:"Modelos que aún no has probado, disponibles para enviar.",en:"Models you haven't tried yet, ready to ship.",it:"Modelli che non hai ancora provato, pronti da spedire."},
  descuentoManual:{fr:"Remise manuelle",es:"Descuento manual",en:"Manual discount",it:"Sconto manuale"},
  anadirDisenoPedido:{fr:"Ajouter un modèle à la commande",es:"Añadir diseño al pedido",en:"Add a design to the order",it:"Aggiungi un modello all'ordine"},
  anadirDisenoSub:{fr:"Tant que la commande n'est pas expédiée, vous pouvez ajouter des modèles disponibles en stock.",es:"Mientras el pedido no esté enviado, puedes añadir diseños disponibles en stock.",en:"While the order isn't shipped, you can add designs available in stock.",it:"Finché l'ordine non è spedito, puoi aggiungere modelli disponibili."},
  elegirDiseno:{fr:"Choisir un modèle",es:"Elegir diseño",en:"Choose a design",it:"Scegli un modello"},
  dispo:{fr:"dispo",es:"disp.",en:"avail.",it:"disp."},
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(24,51,47,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}} onClick={() => setModal(null)}>
        <div style={{background:C.wh,padding:"24px 20px",borderRadius:8,width:"100%",maxWidth:480,maxHeight:"90vh",overflow:"auto",margin:"0 16px"}} onClick={e => e.stopPropagation()}>
          ...
          {modal === "editOrd" && <>
            ...
            {(() => {
              const subtotal = (ed.lines||[]).reduce((s,x)=>s+x.price*(x.qtyServed==null?x.qty:x.qtyServed),0);
              const finalT = applyDiscount(subtotal, ed.discountType, ed.discountValue);
              const saved = subtotal - finalT;
              return <div style={{background:"#faf6ef",border:"1px solid #e8dcc8",borderRadius:8,padding:"12px 14px",marginBottom:14}}>
                <div style={{fontSize:11,fontFamily:BD,color:"#a06a2c",fontWeight:700,marginBottom:8}}>🏷️ {t("descuentoManual")}</div>
                <div style={{display:"flex",gap:6,marginBottom:8}}>
                  {[["","Sin dto."],["pct","%"],["amount","€"]].map(([v,l]) =>
                    <button key={v} onClick={() => setEd(p => ({...p, discountType:v, discountValue: v===""?0:p.discountValue}))} style={{flex:1,padding:"6px 0",background:(ed.discountType||"")===v?C.dk:"transparent",color:(ed.discountType||"")===v?C.bg:C.gr,border:"1px solid "+((ed.discountType||"")===v?C.dk:C.ln),borderRadius:6,fontSize:11,fontFamily:BD,fontWeight:600,cursor:"pointer"}}>{l}</button>)}
                </div>
                {ed.discountType && <input type="number" step="0.01" value={ed.discountValue||""} onChange={e => setEd(p => ({...p, discountValue: parseFloat(e.target.value)||0}))} placeholder={ed.discountType==="pct"?"10 (%)":"50.00 (€)"} style={{width:"100%",padding:9,border:"1px solid "+C.ln,borderRadius:6,fontFamily:BD,fontSize:12,background:C.wh,color:C.dk,boxSizing:"border-box",marginBottom:8}} />}
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11,fontFamily:BD,color:C.gr}}><span>Subtotal</span><span>{fmt(subtotal)} €</span></div>
                {saved > 0.001 && <div style={{display:"flex",justifyContent:"space-between",fontSize:11,fontFamily:BD,color:"#a06a2c",marginTop:3}}><span>Descuento</span><span>− {fmt(saved)} €</span></div>}
                <div style={{display:"flex",justifyContent:"space-between",fontSize:14,fontFamily:BD,fontWeight:700,color:C.dk,marginTop:6,paddingTop:6,borderTop:"1px solid "+C.ln}}><span>Total cliente</span><span>{fmt(finalT)} €</span></div>
              </div>;
            })()}
            ...
            <div style={{display:"flex",gap:8}}>
              <Btn onClick={() => { const subtotal = (ed.lines||[]).reduce((s,x)=>s+x.price*(x.qtyServed==null?x.qty:x.qtyServed),0); const finalT = applyDiscount(subtotal, ed.discountType, ed.discountValue); const updated = {...orders[ed.idx], status:ed.status, pay:ed.pay, track:ed.track, carrier:ed.carrier, trackUrl:ed.trackUrl, notes:ed.notes, clientNotes:ed.clientNotes, lines:ed.lines, items:ed.items, total:finalT, discountType:ed.discountType||"", discountValue:ed.discountValue||0, shippingCost:ed.shippingCost, comm:ed.dist!=="Direct"&&ed.dist!=="Faire"?finalT*0.15:0, payMethod:ed.payMethod, paymentLink:ed.paymentLink, payDueDate:ed.payDueDate, payReminderDays:ed.payReminderDays, paySplit1Amount:ed.paySplit1Amount, paySplit1Date:ed.paySplit1Date, paySplit1Done:ed.paySplit1Done, paySplit2Amount:ed.paySplit2Amount, paySplit2Date:ed.paySplit2Date, paySplit2Done:ed.paySplit2Done}; setOrders(p => p.map((o, i) => i === ed.idx ? updated : o)); dbUpdateOrder(updated); toast(t("enregistrer")); setModal(null); }} style={{flex:1}}>{t("enregistrer")}</Btn>
          ...
          {modal === "viewOrd" && (() => {
            const editableStatus = ed.status !== "shipped" && ed.status !== "delivered" && ed.status !== "cancelled";
            const canEdit = (role === "distributor" && ed.status === "confirmed") || (role === "client" && editableStatus);
            const canClientEdit = role === "client" && editableStatus;
            ...
            {canClientEdit && <div style={{background:CL.gn+"08",border:"1px solid "+CL.gn+"30",borderRadius:10,padding:"12px 14px",marginTop:8,marginBottom:4}}>
              <div style={{fontSize:11,fontFamily:BD,color:CL.gn,fontWeight:700,marginBottom:8}}>➕ {t("anadirDisenoPedido")}</div>
              <div style={{fontSize:10,fontFamily:BD,color:C.gr,marginBottom:10,lineHeight:1.5}}>{t("anadirDisenoSub")}</div>
              <select id="clientAddProd" style={{width:"100%",padding:9,border:"1px solid "+C.ln,borderRadius:6,fontFamily:BD,fontSize:12,background:C.wh,color:C.dk,boxSizing:"border-box",marginBottom:8}}>
                <option value="">— {t("elegirDiseno")} —</option>
                {products.filter(p => p.active!==false && p.stock>0 && p.col!=="Acetato").sort((a,b)=>(a.model+a.color).localeCompare(b.model+b.color)).map(p => <option key={p.id} value={p.id}>{p.model} · {p.color} ({p.stock} {t("dispo")||"disp."})</option>)}
              </select>
              <Btn small onClick={() => {
                const sel = document.getElementById("clientAddProd"); const p = products.find(x => String(x.id) === String(sel.value)); if(!p) return;
                const exists = ed.lines.find(l => l.model===p.model && l.color===p.color);
                let nl;
                if (exists) { nl = ed.lines.map(l => (l.model===p.model && l.color===p.color) ? {...l, qty:l.qty+2, qtyServed:null} : l); }
                else { const unitPrice = activeClient && activeClient.customPrice>0 ? activeClient.customPrice : (p.col==="Acetato"?p.fixedPrice:(essentialUnitPrice||26.90)); nl = [...ed.lines, {model:p.model, color:p.color, sku:p.sku, qty:2, price:unitPrice, col:p.col, qtyServed:null, lineStatus:"complete"}]; }
                const ni = nl.reduce((s,x)=>s+x.qty,0); const nt = nl.reduce((s,x)=>s+x.qty*x.price,0);
                setEd(prev => ({...prev, lines:nl, items:ni, total:nt})); sel.value=""; toast(t("anadidoAlPedido"));
              }} style={{width:"100%"}}>+ {t("anadir")}</Btn>
            </div>}
            {canEdit && <Btn onClick={() => { 
              const orig = orders.find(o => o.id === ed.id);
              const changes = [];
              ...
              const savedOrder = {...orders[ed.idx], lines:ed.lines, items:ed.items, total:ed.total, clientNotes:ed.clientNotes, notes:ed.notes, status:ed.status, track:ed.track, carrier:ed.carrier};
              dbUpdateOrder(savedOrder);
              if (changes.length > 0) logOrderChange(ed.id, role === "client" ? "✏️ Cliente editó su pedido" : "Edición de pedido", (role === "client" ? "["+(ed.client||user.co||user.name)+"] " : "") + changes.join(" · "));
              toast(t("enregistrer"));
              setModal(null); 
      ...
      {view === "c-home" && <div>
        ...
        <div style={{padding:"20px min(24px, 4vw)"}}>
          ...
          {(() => {
            ...
            const lastOrdered = {};
            myOrders.forEach(o => {
              const od = o._rawDate ? new Date(o._rawDate) : (o.date ? new Date(o.date.split("/").reverse().join("-")) : null);
              (o.lines||[]).forEach(l => { const k = l.model+"||"+l.color; if(od && (!lastOrdered[k] || od > lastOrdered[k])) lastOrdered[k] = od; });
            });
            const now = new Date();
            const restock = Object.entries(lastOrdered).map(([k, d]) => {
              const days = Math.floor((now - d)/(1000*60*60*24));
              const [model, color] = k.split("||");
              const prod = products.find(p => p.model===model && p.color===color && p.active!==false && p.stock>0);
              return (days >= 60 && prod) ? {prod, days} : null;
            }).filter(Boolean).sort((a,b) => b.days - a.days).slice(0,6);
            if (restock.length === 0) return null;
            return <div style={{background:"linear-gradient(135deg,#2980b910,#2980b904)",border:"1.5px solid #2980b935",borderRadius:14,padding:"16px 18px",marginBottom:18}}>
              <div style={{fontSize:13,fontFamily:BD,fontWeight:700,color:"#2471a3",marginBottom:4}}>🔄 {t("quizasReponer")}</div>
          ...
          {/* ✨ DESCUBRE (modelos que no ha pedido nunca, con stock, top venta) */}
          {(() => {
            const myOrders = orders.filter(o => o.client === user.co || o.client === user.name);
            const orderedKeys = new Set();
            myOrders.forEach(o => (o.lines||[]).forEach(l => orderedKeys.add(l.model+"||"+l.color)));
            const discover = products.filter(p => p.active!==false && p.stock>0 && !orderedKeys.has(p.model+"||"+p.color) && (p.tags||[]).includes("top")).slice(0,6);
            const pool = discover.length >= 3 ? discover : products.filter(p => p.active!==false && p.stock>5 && !orderedKeys.has(p.model+"||"+p.color)).slice(0,6);
            if (pool.length === 0) return null;
            return <div style={{background:"linear-gradient(135deg,#8e44ad10,#8e44ad04)",border:"1.5px solid #8e44ad30",borderRadius:14,padding:"16px 18px",marginBottom:18}}>
              <div style={{fontSize:13,fontFamily:BD,fontWeight:700,color:"#7d3c98",marginBottom:4}}>✨ {t("descubreModelos")}</div>
